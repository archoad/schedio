...
function readCredentialPublicKey($binary, $offset, &$endOffset)
{
	...
	$_COSE_X = -2;
	$_COSE_Y = -3;
	$enc = WebAuthn\CBOR\CborDecoder::decodeInPlace(
		$binary,
		$offset,
		$endOffset,
	);
...
function generatePKCCOregistration()
{
	...
	return json_encode(["publicKey" => $result]);
}
...
function verifyAttestationData($attestationObject)
{
	...
	if (
		...
		$attStmt["alg"] !== $coseAlgoECDSAwSHA256
	) {
	...
	if ($flags["attestedDataIncluded"]) {
		if (strlen($binAuthData) <= 55) {
			genSyslog(
				__FUNCTION__,
				$msg = "attested data should be present but is missing",
			);
			$success = false;
		}
		$attestationData = readAttestData($binAuthData, $offset);
		$credPKey = readCredentialPublicKey(
			$binAuthData,
			55 + $attestationData["length"],
			$offset,
		);
	...
	$data["authData"]["credentialData"]["publicKey"]["x"] = base64_encode(
		$credPKey["x"],
	);
	$data["authData"]["credentialData"]["publicKey"]["y"] = base64_encode(
		$credPKey["y"],
	);
...
function validateAttestation($clientDataHash, $attestationData)
{
	...
	if ($fmt === "packed") {
		...
		$publicKey = openssl_pkey_get_public($attestationData["certificate"]);
		$x = base64_decode(
			$attestationData["authData"]["credentialData"]["publicKey"]["x"],
		);
		$y = base64_decode(
			$attestationData["authData"]["credentialData"]["publicKey"]["y"],
		);
		$signature = base64_decode($attestationData["attStmt"]["sig"]);
		$dataToVerify = "\x00";
		$dataToVerify .= base64_decode(
			$attestationData["authData"]["rpIdHash"],
		);
		$dataToVerify .= $clientDataHash;
		$dataToVerify .= base64_decode(
			$attestationData["authData"]["credentialData"]["credentialId"],
		);
...
function verifyRegistration($clientDataJSON, $attestationObject)
{
	...
	if (
		!property_exists($clientData, "type") ||
	...
	if (
		!property_exists($clientData, "challenge") ||
		base64_encode(base64UrlDecode($clientData->challenge)) !==
			$_SESSION["challenge"]
	) {
	...
	if (
		!property_exists($clientData, "origin") ||
		!checkOrigin($clientData->origin)
	) {
	...
	if (
		...
		!array_key_exists("fmt", $decodedData) ||
		!is_string($decodedData["fmt"])
	) {
		genSyslog(__FUNCTION__, $msg = "invalid attestation format");
		$success = false;
	}
	$fmt = $decodedData["fmt"];
	$allowedFormats = ["fido-u2f", "packed", "none"];
	...
	if (
		!array_key_exists("authData", $decodedData) ||
		!is_object($decodedData["authData"])
	) {
		genSyslog(
			__FUNCTION__,
			$msg = "invalid attestation format (authData not available)",
		);
		$success = false;
	}
	//Verify the attestation statement format attStmt
	if (
		!array_key_exists("attStmt", $decodedData) ||
		!is_array($decodedData["attStmt"])
	) {
		genSyslog(
			__FUNCTION__,
			$msg = "invalid attestation format (attStmt not available)",
		);
		$success = false;
	}
	if ($fmt !== "none") {
	...
	if (!$attestationData["authData"]["flags"]["UP"]) {
		genSyslog(
			__FUNCTION__,
			$msg = "user not present during authentication",
		);
		$success = false;
	}
	if ($fmt !== "none") {
		if (validateAttestation($clientDataHash, $attestationData) !== 1) {
			genSyslog(
				__FUNCTION__,
				$msg =
					"invalid certificate signature: " . openssl_error_string(),
			);
			$success = false;
		}
	}
	$data["credentialId"] =
		$attestationData["authData"]["credentialData"]["credentialId"];
	$data["credentialPublicKeyPEM"] =
		$attestationData["credentialPublicKeyPEM"];
...
function registerNewCredential($post)
{
	...
	if ($result[0]) {
		$base = dbConnect();
		$request = sprintf(
			"UPDATE users SET credential_id='%s', public_key='%s', sign_count='%d' WHERE login='%s'",
			$result[1]["credentialId"],
			$result[1]["credentialPublicKeyPEM"],
			$result[1]["signCount"],
			$_SESSION["login"],
		);
...
function verifyAssertion(
	...
	$signatureCounter = unpack("Nsigncount", substr($authenticatorData, 33, 4))[
		"signcount"
	];
	if ($signatureCounter > 0) {
		if (
			$_SESSION["registration"]["signCount"] !== null &&
			$_SESSION["registration"]["signCount"] >= $signatureCounter
		) {
...
function validateNewAssertion($post)
{
	...
	if (isset($_SESSION["registration"]["credentialId"])) {
		if ($_SESSION["registration"]["credentialId"] === $post["id"]) {
			$credentialPublicKey =
				$_SESSION["registration"]["credentialPublicKeyPEM"];
		}
		if ($credentialPublicKey === null) {
			$success = false;
		} else {
			$success = verifyAssertion(
				$clientDataJSON,
				$authenticatorData,
				$signature,
				$credentialPublicKey,
