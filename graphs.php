...
{
	$tmp = date_format(date_create($date), "Y-m-d");
	$tmp = $tmp . " 23:59";
	return $tmp;
}
...
function getProjects()
{
	...
	if (intval($_SESSION["role"]) == 3) {
		$request = sprintf(
			"SELECT * FROM project WHERE chef='%d' ",
			intval($_SESSION["uid"]),
		);
	} elseif (intval($_SESSION["role"]) == 2) {
		$request = sprintf(
			"SELECT * FROM project WHERE directeur='%d' ",
			intval($_SESSION["uid"]),
		);
	} else {
	...
	while ($row = mysqli_fetch_object($result)) {
		...
		if (!$closed) {
			$projects[] = [
				...
				"content" => sprintf(
					"<b><a href='user.php?action=mgmt&value=%d'>%s</a></b><br />Avancement: %d%%",
					$row->id,
					traiteStringFromBDD($row->nom),
					computeProjectProgress($row->id),
				),
...
function getTasks($projects)
{
	...
	foreach ($projects as $p) {
		$base = dbConnect();
		$request = sprintf(
			"SELECT * FROM task WHERE projet='%d' ",
			intval($p["id"]) - 100,
		);
		$result = mysqli_query($base, $request);
		dbDisconnect($base);
		while ($row = mysqli_fetch_object($result)) {
			$tasks[] = [
				...
				"style" => sprintf(
					"padding:2px; font-size: 8pt; border-color: %s; background-color: %s;",
					hex2rgb($colors[$counter], "1.0"),
					hex2rgb($colors[$counter], "0.5"),
				),
				"group" => intval($p["id"]),
	...
	foreach ($temp as $chapID => $projID) {
		...
		$chapters[] = [
			...
			"content" => sprintf(
				"<b>%d - %s</b>",
				$row->num,
				traiteStringFromBDD($row->shortname),
			),
			"nestedGroups" => $projID,
			"treeLevel" => 1,
			"showNested" => true,
...
$groups = [];
foreach ($chapters as $elt) {
	$groups[] = $elt;
}
foreach ($projects as $elt) {
	$groups[] = $elt;
}

$rawdata = ["groups" => $groups, "tasks" => $tasks];
